"""Step 5: run the local G1 Manager-Based environment with contact sensors."""
...
from sensors_cfg import DEFAULT_ENV_SPACING, DEFAULT_MAX_STEPS, DEFAULT_NUM_ENVS, CONTACT_SENSOR_NAME
from sensors_cfg import create_g1_sensor_env_cfg


def parse_args() -> argparse.Namespace:
    """Parse Step 5 CLI arguments and IsaacLab app launcher arguments."""
    from isaaclab.app import AppLauncher

    parser = argparse.ArgumentParser(description="Step 5: run the local G1 with contact sensors.")
...
def run_environment(env, simulation_app, max_steps: int) -> None:
    """Run random actions and print contact sensor tensor shapes."""
    import torch

    count = 0
    env.reset()
    print("[INFO]: Step 5 sensor environment reset complete.")
    ...
    while simulation_app.is_running() and (max_steps <= 0 or count < max_steps):
        with torch.inference_mode():
            ...
            if count == 0:
                ...
                print(f"[INFO]: Contact force shape: {tuple(contact_sensor.data.net_forces_w.shape)}")
                print(f"[INFO]: Max contact force: {torch.max(contact_sensor.data.net_forces_w).item():.6f}")
            count += 1
...
def main(args: argparse.Namespace, simulation_app) -> None:
    ...
    env_cfg = create_g1_sensor_env_cfg(num_envs=args.num_envs, env_spacing=args.env_spacing)
...
if __name__ == "__main__":
    ...
    if args_cli.max_steps > 0 and not args_cli.graceful_close:
        print("[INFO]: Step 5 bounded run complete.", flush=True)
        os._exit(0)
